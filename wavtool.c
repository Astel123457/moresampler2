...
int wavtool(wavtool_data* data) {
    ...
    llsm_chunk* input = get_chunk_from_file(data->input, &nfrm_in, &fs_in, &nbit_in, &nx_in, &nhop_in, opt_a_in, opt_s_in);
    if (!input) {
    ...
    llsm_chunk* output = get_chunk_from_file(data->output, &nfrm_out, &fs_out, &nbit_out, &nx_out, &nhop_out, opt_a_out, opt_s_out);
    if (!output) {
